...
parser.add_argument("--gbfile", help="The .gb file that needs to be processed")
...
def execute(input_file_name, fasta_file, tsv_file, log_file, header_list, feature_list, recognition_list):
    ...
    :param input_file_name: gb file that will be parsed                             - REQUIRED
    ...
    :param header_list: Header list                                                 - Not REQUIRED
    ...
    try:
        for index, record in enumerate(SeqIO.parse(input_file_name, "gb")):
                ...
                for indexHeader in range(len(keyValues) + 1):
                    if indexHeader < len(keyValues):
                        ...
                        if len(seqBatch) != 0:
                            try:
                                # Recognition
                                ws.cell(row=workingRow + 1, column=indexHeader + 1,
                                        value=str(seqBatch[relativeLine][1][0]))
                                # Qualifiers found
                                ...
                                # Location
                                ws.cell(row=workingRow + 1, column=indexHeader + 3,
                                        value=str(seqBatch[relativeLine][3][0]))
                                # Sequence
                                ws.cell(row=workingRow + 1, column=indexHeader + 4,
                                        value=str(seqBatch[relativeLine][4][0]))
                                # IsPseudo
                                if len(seqBatch[relativeLine][5]) == 0:
                                    ws.cell(row=workingRow + 1, column=indexHeader + 5, value="")
                                else:
                                    ws.cell(row=workingRow + 1, column=indexHeader + 5,
                                            value=str(seqBatch[relativeLine][5][0]))
